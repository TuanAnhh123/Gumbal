...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('jail')
        .setDescription('Put someone to jail')
    ...
    async execute(interaction) {
        ...
        {
            fetch(`https://some-random-api.ml/canvas/jail?avatar=${user.displayAvatarURL({dynamic : true , format : 'png'})}&key=s1HpWsm7B8J56EZoZjwZsb7nhbZhYqZmRezxQGznVS7xC60Vo9dvGoDrZd8NKzCZ`)
                .then(async response => Buffer.from(await response.arrayBuffer()))
                .then(data => {
                    interaction.reply({files: [new Discord.MessageAttachment(data, "jail.png")]
                 })
               });
        }
    }
