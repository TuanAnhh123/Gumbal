...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('mission-completed')
        .setDescription('Respect them')
    ...
    async execute(interaction) {
        ...
        {
            fetch(`https://some-random-api.ml/canvas/passed?avatar=${user.displayAvatarURL({dynamic : true , format : 'png'})}&key=s1HpWsm7B8J56EZoZjwZsb7nhbZhYqZmRezxQGznVS7xC60Vo9dvGoDrZd8NKzCZ`)
                .then(async response => Buffer.from(await response.arrayBuffer()))
                .then(data => {
                    interaction.reply({files: [new Discord.MessageAttachment(data, "mip.png")]
                 })
               });
        }
    }
