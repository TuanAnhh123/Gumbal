...
module.exports = {
    data : new SlashCommandBuilder()
        .setName('remove-music-channel')
        .setDescription('Remove music request channel'),
        ...
        try {
            ...
            if(db.fetch(`${guild}_music_channel`) === null)
            {
                const embed = {
                    ...
                    description : '<:false:964905677518696548> This server has no request channel.',
                    color : 'BLUE',
                    timestamp: new Date(),
                    footer: {
                        text: `${interaction.user.tag}`,
            ...
            const channel = await db.get(`${guild}_music_channel`);

            await db.delete(`${guild}_music_channel` , channel)
            ...
            const embed = {
                ...
                description : `<:remove:967328034652848168> Removed music request channel : <#${channel}>.`,
                color : 'BLUE',
                timestamp: new Date(),
                footer: {
                    text: `${interaction.user.tag}`,
